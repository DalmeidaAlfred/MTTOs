...
send_http_post() {
    ...
    if [ $? -eq 0 ]; then
        sudo echo "$(date): HTTP POST request sent successfully" >> /home/pi/PIMCO/tv_status_check.log
    else
        sudo echo "$(date): HTTP POST request failed" >> /home/pi/PIMCO/tv_status_check.log
    fi
}
