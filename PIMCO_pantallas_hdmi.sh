...
send_http_post() {
    ...
    if [ $? -eq 0 ]; then
        echo "$(date): HTTP POST request sent successfully" >> /home/pi/PIMCO/hdmi_status_check.log
    else
        echo "$(date): HTTP POST request failed" >> /home/pi/PIMCO/hdmi_status_check.log
    fi
}
